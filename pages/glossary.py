...
with st.expander("💎 班傑明·葛拉漢 (Benjamin Graham) - 防禦型價值"):
    ...
    * **在地化修正:** 為了平滑台股電子業的景氣循環，本系統強制使用 **「過去 5 年 (20 季) 平均 EPS」**，而非單一年度獲利。這能避免在景氣最高峰誤判股價便宜。
...
with st.expander("📈 月營收動能 (Monthly Revenue Alpha)"):
    st.markdown("""
    [cite_start]**差異分析[cite: 278]:**
    * **美股：** 依賴季報 (10-Q)，資訊落後。
    * **台股：** 每月 10 日強制公佈營收，具有極強的領先性。
    
    **策略邏輯：** * **營收雙強：** 當 **年增率 (YoY) > 20%** 且 **月增率 (MoM) > 0%** 時，代表長線成長趨勢確立且短線動能未歇，是台股最強的 Alpha 訊號之一 (+2 分)。
    """)
...
with st.expander("💰 三大法人籌碼 (Institutional Chips)"):
    ...
    * **台股：** 每日盤後揭露三大法人買賣超。
    ...
    [cite_start]**策略邏輯 [cite: 323, 330-334]:**
    * **外資 (Foreign):** 資金量大，**連續 3 日買超** 通常代表波段趨勢 (+1 分)。
    * **投信 (Trust):** 追求績效，偏好認養 **中小型成長股** (股本 < 50億)。當投信近期積極買超這類股票時，往往隱含作帳行情 (+2 分)。
    """)

with st.expander("⚠️ 流動性陷阱 (Liquidity Trap)"):
    st.markdown("""
    **風險定義：** 台股中小型股容易出現「無量下跌」或「跌停鎖死」的情況。
    **策略邏輯：** 若日均成交量 **< 500 張**，系統將視為流動性風險過高，直接 **扣 2 分**，避免買到想賣賣不掉的股票。
    """)
...
st.markdown("""
#### [cite_start]🛡️ 延遲與滑價對策 [cite: 346-351]
